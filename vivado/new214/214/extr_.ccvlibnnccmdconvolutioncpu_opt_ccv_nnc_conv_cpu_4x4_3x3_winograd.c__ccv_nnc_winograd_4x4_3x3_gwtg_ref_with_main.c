...
__attribute__((used)) inline static void _ccv_nnc_winograd_4x4_3x3_gwtg_ref(volatile const float* const w, const int c,volatile float* gwtg)
{
	int i;
	for (i = 0; i < c; i++)
	{
...
}/* Main routine for _ccv_nnc_winograd_4x4_3x3_gwtg_ref */
int main() {
  const float *const w;
  const int c;
  float * gwtg;
