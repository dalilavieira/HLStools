...
configure_resizer_out_params(volatile struct vpfe_resizer_device *resizer, int index,
			     volatile void *output_spec, unsigned char partial,
			     unsigned int flag)
{
	...
	param->ext_mem_param[index].user_c_ofst =
					(output->user_c_ofst + 31) & ~0x1f;
}/* Main routine for configure_resizer_out_params */
int main() {
