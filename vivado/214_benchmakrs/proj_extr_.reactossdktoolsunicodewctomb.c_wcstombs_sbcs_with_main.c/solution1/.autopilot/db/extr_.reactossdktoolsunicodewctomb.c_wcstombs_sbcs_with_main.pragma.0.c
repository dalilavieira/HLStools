...
struct sbcs_table {unsigned char uni2cp_low[1000]; unsigned short uni2cp_high[1000]; } ;
typedef int WCHAR ;



int wcstombs_sbcs( volatile const struct sbcs_table *table,
                                 const WCHAR src[1000], unsigned int srclen,
                                 char dst[1000], unsigned int dstlen )
{_ssdm_SpecArrayDimSize(src, 1000);_ssdm_SpecArrayDimSize(dst, 1000);
