...
int partition (__int32_t arr[100], int low, int high)
{_ssdm_SpecArrayDimSize(arr, 100);
    int pivot = arr[high];
    int i = (low - 1);
...
void quickSort(__int32_t arr[], int low, int high)
{
    if (low < high)
    {
...
void printArray(__int32_t arr[100], int size)
{_ssdm_SpecArrayDimSize(arr, 100);
    int i;
    for (i=0; i < size; i++)
        printf("%d ", arr[i]);
