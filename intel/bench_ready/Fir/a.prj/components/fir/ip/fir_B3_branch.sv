...
module fir_B3_branch (
    ...
    begin
        ...
        else if (fir_B3_branch_enable_q == 1'b1)
        begin
    ...
    // fir_B3_branch_enable(LOGICAL,3)
    assign fir_B3_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // VCC(CONSTANT,1)
    assign VCC_q = $unsigned(1'b1);

    // fir_B3_branch_enable_not(LOGICAL,4)
    assign fir_B3_branch_enable_not_q = ~ (fir_B3_branch_enable_q);

    // out_stall_out(GPOUT,15)
    assign out_stall_out = fir_B3_branch_enable_not_q;
