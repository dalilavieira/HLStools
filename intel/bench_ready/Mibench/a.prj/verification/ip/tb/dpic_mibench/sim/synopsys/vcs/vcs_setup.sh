...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [dpic_mibench::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {dpic_mibench::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [dpic_mibench::get_design_files "$QSYS_SIMDIR"]]
