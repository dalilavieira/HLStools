...
TOP_LEVEL_NAME="sobel_filter_en_cfan"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [sobel_filter_en_cfan::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {sobel_filter_en_cfan::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [sobel_filter_en_cfan::get_design_files "$QSYS_SIMDIR"]]
