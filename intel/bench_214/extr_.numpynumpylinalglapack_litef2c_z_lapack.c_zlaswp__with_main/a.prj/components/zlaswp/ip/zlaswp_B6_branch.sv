...
// SystemVerilog created from zlaswp_B6_branch
...
module zlaswp_B6_branch (
    input wire [31:0] in_ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31,
    ...
    wire [0:0] VCC_q;
    reg [31:0] ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31_reg_q;
    ...
    wire [0:0] zlaswp_B6_branch_enable_q;
    wire [0:0] zlaswp_B6_branch_enable_not_q;
    ...
    begin
        ...
        else if (zlaswp_B6_branch_enable_q == 1'b1)
        begin
    ...
    // zlaswp_B6_branch_enable(LOGICAL,44)
    assign zlaswp_B6_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (zlaswp_B6_branch_enable_q == 1'b1)
        begin
            ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31_reg_q <= in_ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31;
        end
    end

    // out_ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31(GPOUT,28)
    assign out_ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31 = ZZ7zlaswp_PiP8TYPE_3_S_S_S_S_S_E2ix_static_1_replace_phi221_pop31_reg_q;

    // lm3213_toi1_intcast56_reg(REG,15)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_memdep_102(GPOUT,30)
    assign out_memdep_102 = memdep_102_reg_q;

    // memdep_119_reg(REG,18)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_memdep_119(GPOUT,31)
    assign out_memdep_119 = memdep_119_reg_q;

    // memdep_71_reg(REG,19)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_memdep_71(GPOUT,32)
    assign out_memdep_71 = memdep_71_reg_q;

    // memdep_87_reg(REG,20)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_memdep_87(GPOUT,33)
    assign out_memdep_87 = memdep_87_reg_q;

    // notcmp193223_pop33_reg(REG,27)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_notcmp193223_pop33(GPOUT,34)
    assign out_notcmp193223_pop33 = notcmp193223_pop33_reg_q;

    // phi_decision209_xor_and_i0222_pop32_reg(REG,39)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_phi_decision209_xor_and_i0222_pop32(GPOUT,35)
    assign out_phi_decision209_xor_and_i0222_pop32 = phi_decision209_xor_and_i0222_pop32_reg_q;

    // zlaswp_B6_branch_enable_not(LOGICAL,45)
    assign zlaswp_B6_branch_enable_not_q = ~ (zlaswp_B6_branch_enable_q);

    // out_stall_out(GPOUT,36)
    assign out_stall_out = zlaswp_B6_branch_enable_not_q;
