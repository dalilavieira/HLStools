...
echo "Benchmark, number_line, size_compile (KBytes)" > "results.csv"

#set -e

for ((i=0; i < ${#BENCHMARKS[@]}; i++)) do
    echo "RUNNING $i: "${BENCHMARKS[i]}
    gcc -c "214_benchmarks/"${BENCHMARKS[i]}"/"${BENCHMARKS[i]}".c"
    python3 get_size.py ${BENCHMARKS[i]}".c" >> "results.csv"
    rm ${BENCHMARKS[i]}".o"
done